...
def save_plot(*args):
	...
	filename = "{}\\..\\{}_{}.png".format(os.getcwd(),"freq_dist_polygon",time.strftime('%y%m%d%H%M%S', time.localtime(time.time())))
	plt.savefig(filename)
	return filename
	# print(filename)

def test():
	save_plot('','디스이즈 스파르타!','(a)','(b)', ",".join(list(map(lambda x:str(x), (np.random.randn((20))*5+30).tolist()))), 'url.png')
